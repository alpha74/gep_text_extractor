...
def get_text_rect( img, show_details = 1 ):
	...
	bottom_right = [ 0, 0 ]			# Belongs to good_indices. Indicates the top_right corner of text.
	...
	# Finding bottom_right
	for x in range ( int(img_width * 0.75) ,img_width ):		
		for y in range ( int(img_height * 0.6 ), img_height  ):
	...
	cv2.circle( temp_img, ( bottom_right[0],bottom_right[1]), 2, (255,0,0), -1 )
	cv2.imshow( "Hit Points", temp_img )
		
	print( "TL: " + str( top_left ) + "  TR: " + str( bottom_right ) )
